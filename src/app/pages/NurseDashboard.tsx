...
const INITIAL_ALERTS: IncidentAlert[] = [
  {
    id: 'evt-1',
    time: new Date(Date.now() - 2 * 60 * 1000).toTimeString().split(' ')[0],
    timestamp: Date.now() - 2 * 60 * 1000,
    camera: '복도 A', type: 'FALL', label: 'FALL (낙상) 감지', severity: 'critical', status: 'new',
  },
  {
    id: 'evt-2',
    time: new Date(Date.now() - 6 * 60 * 1000).toTimeString().split(' ')[0],
    timestamp: Date.now() - 6 * 60 * 1000,
    camera: '방 1', type: 'FAINT', label: 'FAINT (실신) 감지', severity: 'warning', status: 'new',
  },
  {
    id: 'evt-3',
    time: new Date(Date.now() - 15 * 60 * 1000).toTimeString().split(' ')[0],
    timestamp: Date.now() - 15 * 60 * 1000,
    camera: '대기실 1', type: 'CROWD', label: 'CROWD (혼잡) 감지', severity: 'info', status: 'resolved',
  },
  {
    id: 'evt-4',
    time: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toTimeString().split(' ')[0],
    timestamp: Date.now() - 2 * 24 * 60 * 60 * 1000,
    camera: '출입구', type: 'CROWD', label: 'CROWD (혼잡) 감지', severity: 'info', status: 'resolved',
  },
  {
    id: 'evt-5',
    time: new Date(Date.now() - 12 * 24 * 60 * 60 * 1000).toTimeString().split(' ')[0],
    timestamp: Date.now() - 12 * 24 * 60 * 60 * 1000,
    camera: '후문', type: 'FIRE', label: 'FIRE (화재 연기) 감지', severity: 'critical', status: 'resolved',
  }
];

const INITIAL_CAMERAS: RegisteredCamera[] = [
  { id: 'CCTV-01', name: '방 1', location: '1층' },
  { id: 'CCTV-02', name: '복도 A', location: '1층' },
  { id: 'CCTV-03', name: '방 2', location: '1층' },
];

const ALL_MENU_ITEMS = [
  { id: 'home',       label: '대시보드 홈',    icon: Tv,          individualOnly: false },
  { id: 'monitoring', label: '실시간 모니터링', icon: Video,       individualOnly: false },
  ...
  { id: 'settings',   label: '설정',          icon: Settings,    individualOnly: false },
...
export function NurseDashboard({ username, userType, onLogout }: NurseDashboardProps) {
  ...
  const handleAddCamera = () => {
    ...
    setRegisteredCameras(prev => [...prev, { id: newCamId.trim(), name: newCamName.trim(), location: newCamLocation.trim() || '미지정' }]);
    setNewCamName(''); setNewCamId(''); setNewCamLocation('');
        ...
        <main className="flex-1 flex overflow-hidden">
                  ...
                  {CCTV_FEEDS.map(cam => (
                    ...
                    >
                      <div className="relative aspect-video bg-black overflow-hidden">
                        ...
                        <span className="absolute bottom-2 left-2 text-[10px] text-white font-bold bg-slate-900/80 px-2 py-0.5 rounded">
                          {cam.name}
                        </span>
                        {cam.alert && (
                    ...
                    {alerts.slice(0, 5).map(evt => (
                      <div key={evt.id} className={`bg-[#0f172a] rounded-xl p-3 flex items-center gap-3 transition-opacity ${evt.status === 'resolved' ? 'opacity-50' : ''}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className="text-white font-bold text-sm leading-tight cursor-pointer hover:underline truncate"
          ...
          {activeMenu === 'history' && (
            <div className="flex-1 p-6 space-y-6 overflow-y-auto max-w-5xl flex flex-col">
              ...
              <div className="bg-[#071329] border border-slate-800 p-4 rounded-2xl space-y-4">
              ...
              <div className="flex-1 bg-[#071329] border border-slate-800 rounded-2xl overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-slate-800">
                  {filteredHistory.length === 0 ? (
                    ...
                    <div key={log.id} className="p-4 flex items-center justify-between hover:bg-slate-800/10 transition-colors">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => alert('MP4 저장 완료')} className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-white rounded border border-slate-800 transition-all cursor-pointer" title="다운로드"><Download className="w-3.5 h-3.5" /></button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg flex items-center gap-2 transition-colors cursor-pointer"
                >
              ...
              <div className="bg-[#071329] border border-slate-800 rounded-2xl p-5">
                <h3 className="text-sm font-bold text-white mb-4">실시간 CCTV 모니터링</h3>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div key={cam.id} className="bg-[#111827] border border-slate-800 rounded-xl overflow-hidden group">
                      <div className="relative aspect-video">
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 p-1 bg-slate-900/80 hover:bg-red-600 text-slate-400 hover:text-white rounded transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
                          title="삭제"
                        >
                      ...
                      <div className="p-3">
                        ...
                        <p className="text-slate-400 text-xs mt-0.5">{cam.id} · {cam.location}</p>
                      </div>
                    </div>
                  ))}
                  ...
                  {Array(Math.max(0, 6 - registeredCameras.length)).fill(null).map((_, i) => (
                    <div
                      key={i}
                      className="bg-[#111827] border border-dashed border-slate-700 rounded-xl aspect-video flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-slate-500 hover:bg-slate-800/30 transition-all"
                      onClick={() => setShowAddCamera(true)}
                    >
          ...
          {/* ===== SETTINGS / QNA (placeholder) ===== */}
          {(activeMenu === 'settings' || activeMenu === 'qna') && (
            <div className="flex-1 flex items-center justify-center text-slate-500">
              <div className="text-center">
                {activeMenu === 'settings' ? <Settings className="w-12 h-12 mx-auto mb-3 opacity-30" /> : <HelpCircle className="w-12 h-12 mx-auto mb-3 opacity-30" />}
                <p className="text-sm font-semibold">{activeMenu === 'settings' ? '설정 페이지 준비 중' : '문의 게시판 준비 중'}</p>
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-[#071329] border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
            <div className="px-5 py-4 bg-[#061224] border-b border-slate-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowAddCamera(false)} className="text-xs text-slate-400 hover:text-white cursor-pointer">닫기</button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <button onClick={handleAddCamera} className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-xs transition-colors cursor-pointer">등록 완료</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4">
          <div className="w-full max-w-3xl bg-[#071329] border border-slate-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="bg-[#061224] p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button onClick={() => setIsPlaying(p => !p)} className="p-2 hover:bg-slate-800 rounded-xl text-slate-300 hover:text-white transition-all cursor-pointer">
